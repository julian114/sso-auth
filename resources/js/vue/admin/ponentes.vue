            ...
            <div class="row align-items-center">
                <div class="col-md-12">
                    ...
                    <ul class="breadcrumb">
                        ...
                        <li class="breadcrumb-item" aria-current="page">Ponentes</li>
                    </ul>
                </div>
            </div>
        </div>
                    ...
                    <EasyDataTable
                        ...
                        <template #item-opciones="{ id }">
                            <div class="dropdown">
                                ...
                                <li>
                                    <a class="dropdown-item" href="#" @click.prevent="deletePonente(id)">
                ...
                <div class="modal-body">
                    <VForm ref="form_ref" v-slot="{ errors }" @submit="update|create">
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                <div class="form-floating">
                                    <VField name="genero_modal" as="select" class="form-select" :class="{ 'is-invalid': errors.genero_modal }"
                                            id="genero_modal" v-model="genero_modal" rules="required">
                                        <option value="" selected disabled>Selecciona un género</option>
                                        <option v-for="genero in cat_genero" :key="genero" :value="genero">
                                            {{ genero }}
                                        </option>
                                    </VField>
                                    <label for="genero_modal">Género</label>
                                    <VErrorMessage name="genero_modal" class="text-danger mt-1 d-block small" />
                                </div>
                            </div>
                            <div class="col-md-12 mb-3">
                                <div class="form-floating">
...
export default {
    ...
    computed: {
        ponentesFiltrados() {
            ...
            if (!query) return this.ponentes;

            const normalizedQuery = normalizeText(query);

            return this.ponentes.filter(item => {
    ...
    methods: {
        ...
        async get() {
            try {
                ...
                const response = await this.$axios.get('/Ponentes/get');
                if (response.data.status) {
                    this.ponentes = response.data.data.map(item => {
                        return {
                            ...item,
        ...
        async getInstituciones() {
            try {
                this.incrementarCarga();
                const response = await this.$axios.get('/Ponentes/getInstituciones');
                if (response.data.status) {
        ...
        /* Métodos para modal de Ponentes */
        nuevoPonente() {
            this.resetForm();
            this.getInstituciones();
            this.modalTitle = "Nuevo Ponente";
            this.flag = 0;
            this.abrirmodal();
        },
        ...
        async create() {
            try {
                ...
                const data = {
                    ...
                    genero: this.genero_modal,
                    institucion_id: this.institucion_id_modal
                };

                const response = await this.$axios.post('/Ponentes/create', data);
                if (response.data.status) {
                    this.get();
                    this.cerrarmodal();
                    this.resetForm();
                ...
                if (error.response && error.response.data && error.response.data.message) {
                    ...
                    this.$swal.fire('Error', 'Se produjo un error al crear el ponente.', 'error');
                }
            } finally {
                this.decrementarCarga();
            }
        ...
        async edit(id) {
            try {
                ...
                const response = await this.$axios.get(`/Ponentes/edit/${id}`);
                if (response.data.status) {
                    const ponente = response.data.data;
                    this.idponente = ponente.id;
                    this.nombre_modal = ponente.nombre;
                    this.apellido_paterno_modal = ponente.apellido_paterno;
                    this.apellido_materno_modal = ponente.apellido_materno;
                    this.grado_academico_modal = ponente.grado_academico;
                    this.genero_modal = ponente.genero;
                    this.institucion_id_modal = ponente.institucion_id;

                    this.flag = 1;
                    this.modalTitle = "Editar Ponente";
                    this.abrirmodal();
                } else {
                    this.$swal.fire('Error', response.data.message, 'error');
                }
                ...
                if (error.response && error.response.data && error.response.data.message) {
                    ...
                    this.$swal.fire('Error', 'Se produjo un error al obtener los datos del ponente.', 'error');
                }
            } finally {
                this.decrementarCarga();
            }
        ...
        async update() {
            try {
                ...
                const response = await this.$axios.put(`/Ponentes/update/${this.idponente}`, data);
                if (response.data.status) {
                    this.get();
                    this.cerrarmodal();
                    this.resetForm();
                ...
                if (error.response && error.response.data && error.response.data.message) {
                    ...
                    this.$swal.fire('Error', 'Se produjo un error al actualizar el ponente.', 'error');
                }
            } finally {
                this.decrementarCarga();
            }
        ...
        async updateStatus(id) {
            try {
                this.incrementarCarga();
                const response = await this.$axios.put(`/Ponentes/updateStatus/${id}`);
                if (response.data.status) {
                    this.get();
                    this.$swal.fire('Éxito', response.data.message, 'success');
                } else {
                ...
                if (error.response && error.response.data && error.response.data.message) {
                    ...
                    this.$swal.fire('Error', 'Se produjo un error al cambiar el estado del ponente.', 'error');
                }
            } finally {
                this.decrementarCarga();
            }
        ...
        async deletePonente(id) {
            try {
                ...
                if (result.isConfirmed) {
                    this.incrementarCarga();
                    const response = await this.$axios.delete(`/Ponentes/delete/${id}`);
                    if (response.data.status) {
                        this.get();
                ...
                if (error.response && error.response.data && error.response.data.message) {
                    ...
                    this.$swal.fire('Error', 'Se produjo un error al eliminar el ponente.', 'error');
                }
            } finally {
                this.decrementarCarga();
            }
